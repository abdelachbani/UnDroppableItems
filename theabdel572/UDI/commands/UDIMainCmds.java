...
public class UDIMainCmds implements CommandExecutor {
					...
					if (args[0].equalsIgnoreCase("additem")) {
						...
						if (item != null && item.getType() != Material.AIR) {
							...
							if (meta.hasDisplayName()) {
								if (meta.hasLore()) {
									config.set("Items." + item.getType().toString() + "." + meta.getDisplayName()
											+ ".lore", meta.getLore());
									plugin.saveConfig();
									player.sendMessage(plugin.getUDIName() + ChatColor.GREEN
						...
						player.sendMessage(plugin.getUDIName() + ChatColor.GREEN + "The plugin version is: "
	...
	public void sendUDIHelp(Player player) {
		...
		player.sendMessage(
				ChatColor.translateAlternateColorCodes('&', "&a/udi reload &0- &2Use this to reload the config file."));
		player.sendMessage(ChatColor.translateAlternateColorCodes('&',
				"&a/udi additem &0- &2Use this to add the item in your to the undroppable items list."));
		player.sendMessage(ChatColor.translateAlternateColorCodes('&',
				"&a/udi removeitem &0- &2Use this to remove the item in your  the undroppable items list."));
	}
}