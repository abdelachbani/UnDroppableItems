					...
					if(args[0].equalsIgnoreCase("additem")) {
						...
						if(item != null && item.getType() != Material.AIR) {
							...
							if(meta.hasDisplayName()) {
								if(meta.hasLore()) {
									config.set("Items."+item.getType().toString()+"."+meta.getDisplayName()+".lore", meta.getLore());
									plugin.saveConfig();
									player.sendMessage(plugin.getUDIName()+ChatColor.GREEN+"Item added successfully to UnDroppableItems list!");
								}else {
									config.set("Items."+item.getType().toString()+"."+meta.getDisplayName()+".name", "");
									plugin.saveConfig();
									player.sendMessage(plugin.getUDIName()+ChatColor.GREEN+"Item added successfully to UnDroppableItems list!");
								}
							}else {
						...
						if(item != null && item.getType() != Material.AIR) {
							...
							if(meta.hasDisplayName()) {
								if(meta.hasLore()) {
									config.set("Items."+item.getType().toString()+"."+meta.getDisplayName(), null);
									plugin.saveConfig();
									plugin.reloadConfig();
									player.sendMessage(plugin.getUDIName()+ChatColor.GREEN+"Item removed successfully from UnDroppableItems list!");
								}else {
									config.set("Items."+item.getType().toString()+"."+meta.getDisplayName(), null);
									plugin.saveConfig();
									plugin.reloadConfig();
